...
// CancelAllByRepository 批量取消仓库下所有 queued + running 任务
// 路由: POST /api/repositories/:id/tasks/cancel-all
// 返回: { "canceled": N, "message": "..." }
// 即使部分任务取消失败，已经成功的仍会生效，canceled 字段为实际成功数。
func (h *TaskHandler) CancelAllByRepository(c *gin.Context) {
	repoID, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid repository id"})
		return
	...
	count, err := h.service.CancelAll(uint(repoID))
	if err != nil {
		// 即使有错误，count 仍可能 > 0（部分成功）
