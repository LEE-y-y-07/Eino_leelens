...
本次实现为 LeeLens 系统增加了 MCP (Model Context Protocol) 工具调用能力，使 LLM Client 能够向大模型传递 Tools 定义，并处理模型返回的 Tool Calls 请求。
...
测试结果：
```
ok  	gitee.com/li-yuyanglee/leelens-backend/internal/pkg/llm	0.854s
ok  	gitee.com/li-yuyanglee/leelens-backend/internal/pkg/llm/tools	1.470s
```
